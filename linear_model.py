...
def train_ridgeCV(train_feature_df: pd.DataFrame, train_score_df: pd.DataFrame, alphas: np.array) -> Tuple[float, Ridge]:
    """
...
def train_lassoCV(train_feature_df: pd.DataFrame, train_score_df: pd.DataFrame) -> Tuple[float, Lasso]:
    """
    Train the model with the given inputs and returns the trained model.
